...
def formater_date_jjmmaaaa(date_series):
    ...
    En mode CSV, les dates arrivent sous forme de texte au format français 'JJ/MM/AAAA'.
    On les parse avec dayfirst=True puis on retourne des datetime natifs pour que
    mettre_en_forme_excel() puisse forcer le format DD/MM/YYYY dans l'export Excel.
    """
    ...
    def convertir_valeur(val):
        ...
        # Parsing classique (gère 'JJ/MM/AAAA', 'JJ/MM/AAAA HH:MM', etc.)
        # dayfirst=True est fiable ici car les CSV contiennent les dates au format français.
        try:
...
def charger_famille_hexagone(dossier, mots_cles, colonnes_validation):
    """Charge et fusionne plusieurs fichiers Hexagone CSV selon une liste de mots-clés."""
    dfs = []
    fichiers_deja_charges = set()
    for mot in mots_cles:
        fichiers = [f for f in dossier.iterdir() if f.is_file() and mot.lower() in f.name.lower() and f.suffix.lower() == '.csv' and not f.name.startswith('~')]
        for f in fichiers:
            chemin_normalise = f.resolve()
            if chemin_normalise in fichiers_deja_charges:
                continue
            try:
                # En CSV, les fichiers Hexagone ont 2 lignes d'en-tête (Titre du rapport, ligne vide)
                # avant les vraies colonnes → skiprows=2
                df = pd.read_csv(f, sep=';', dtype=str, encoding='latin-1', skiprows=2)
                valider_colonnes(df, colonnes_validation, f.name)
                df['Fichier Source'] = f.name
                logging.info(f"Fichier Hexagone CSV chargé : {f.name}")
                dfs.append(df)
    ...
    if not dfs:
        raise FileNotFoundError(f"Aucun fichier Hexagone CSV trouvé pour les mots-clés : {mots_cles}")
...
def lancer_conciliation(orbis_path=None, hexa_hospit_paths=None, hexa_seances_paths=None, export_dir=None):
    ...
    if orbis_path:
        ...
        fichiers_orbis = [f for f in DOSSIER_IMPORT.iterdir() if f.is_file() and 'orbis' in f.name.lower() and f.suffix.lower() == '.csv' and not f.name.startswith('~')]
    ...
    # Lecture CSV avec séparateur ; et encodage utf-8-sig (pour gérer le BOM Windows)
    df_orbis_brut = pd.read_csv(chemin_orbis, sep=';', dtype=str, encoding='latin-1')
    ...
    if hexa_hospit_paths:
        ...
        for p in hexa_hospit_paths:
            fp = Path(p)
            df = pd.read_csv(fp, sep=';', dtype=str, encoding='latin-1', skiprows=2)
